...
class BrokenSword():
    def __init__(self, is_full):
        ...
        # 鼠标指针图标
        self.mousee = pygame.image.load(
            "%s" % resource_path(os.path.join("ImageTxtSource", "mousee.png"))).convert_alpha()
    ...
    def Main_interfa(self):
        ...
        background = pygame.image.load(
            '%s' % resource_path(os.path.join("ImageTxtSource", "map_1.png"))).convert_alpha()
    ...
    def gamestart(self):
        ...
        # 创建线程并调用
        KKEY = threading.Thread(target=self.ggame)
        TTIME = threading.Thread(target=self.tim)
        # 主线程退出，子线程也退出
        KKEY.daemon = True
        TTIME.daemon = True
        KKEY.start()
    ...
    def tim(self):
        ...
        if self.game == False:
            return
        global timer
        timer = threading.Timer(1, self.tim)
        timer.start()
